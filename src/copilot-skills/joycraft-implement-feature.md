...
When no `todo` specs remain, run the once-per-feature finisher yourself: read and follow `.github/skills/joycraft-session-end/SKILL.md`. It owns the gates the chain deliberately skipped: full validation (must pass before anything graduates `in-review → done`), discovery consolidation, and push/PR per the project's AGENTS.md git autonomy rules.
