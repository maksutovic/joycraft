...
Every installed skill (`.omp/skills/**/SKILL.md`) must declare `entry: human | agent | situational` in its frontmatter — this is presentation/discovery metadata only, not a flow change:
...
2. **Skill files** — glob `.omp/skills/**/*.md`. Count lines per file. Threshold: ≤200 lines each.
...
5. When the overhead report carries two or more questions for the human, the hosted render is an interactive checkpoint instead: build it per `docs/templates/reference/interactive-checkpoint.md` and read the answers back before stamping. The local file stays the canonical render.
...
### Recommendations
- [Specific, actionable items for anything over threshold or non-KEEP disposition — e.g. "CLAUDE.md is 312 lines — split reference sections into docs/"; "PROBATION: 2 hardened rules provisioned under a different model — re-verify via /skill:joycraft-harden"; "MODEL_SUPERSEDED: 'never use markdown' contradicts *Formatting When Appropriate* in the model profile — RETIRE candidate, remove by hand"]
```
