...
```
Run run `/clear` in the CLI, or press Cmd+N (Ctrl+N on Windows/Linux) for a new thread in the desktop/IDE app first. With intent only (no draft brief), substitute `docs/intent/<slug>.md` for the brief path on the command line and the picking-up line of the briefing below — new-feature pre-fills from the intent and writes the brief itself.
