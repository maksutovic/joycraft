...
Now that `docs/areas/` exists, the next `npx joycraft@latest upgrade` (or any future `npx joycraft@latest init`) will pick it up and add the **Areas pointer** to CLAUDE.md automatically — that pointer tells Claude "when working on the X area, read docs/areas/X/README.md first."

Tell the user:

> "Run `npx joycraft@latest upgrade` to refresh CLAUDE.md with the Areas pointer (or `npx joycraft@latest init` if you haven't initialized yet)."
