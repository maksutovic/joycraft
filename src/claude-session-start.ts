/**
 * Source for the Claude SessionStart adapter installed beside the existing
 * Joycraft hook path. It delegates all policy, cache, and registry work to the
 * shared checker and keeps hook failures non-blocking.
 */
export function generateClaudeSessionStartAdapter(): string {
  return `// Joycraft update discovery adapter — runs on Claude Code session start
...
try {
  ...
  if (process.env.CLAUDE_ENV_FILE) {
    const assignment = "export JOYCRAFT_SESSION_ID='" + session.replaceAll("'", ${JSON.stringify("'\"'\"'")}) + "'\\n";
