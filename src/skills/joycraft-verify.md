...
If the user provided a spec path (e.g., `{{skill_prefix}}verify docs/features/<slug>/specs/add-widget.md`), use that path directly.
...
> No specs found under `docs/features/*/specs/` or `docs/bugfixes/`. Please provide a spec path: `{{skill_prefix}}verify path/to/spec.md`
...
2. The project's {{boundary_file}} (look for test/build commands in the Development Workflow section)
...
- **All PASS:** Suggest committing and opening a PR, or running `{{skill_prefix}}session-end` to capture discoveries.
- **Some FAIL:** List the failed criteria and suggest the user fix them, then run `{{skill_prefix}}verify` again.
