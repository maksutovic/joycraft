...
Stamp the chosen route word into the intent's `Status:` line in place (`Status: <route>`), so a later reader can tell routed-to-bugfix from routed-to-backlog; change nothing else in the file. The intent file stays in `docs/intent/` on every route, `discard` included — never delete, move, or archive it. Routing is a stamp plus a handoff, never an execution: `interview` continues into Step 1 with that intent as context; `bugfix` names the next command for the human (`{{skill_prefix}}bugfix docs/intent/<file>.md`); `backlog` names the next step for the human — a user-confirmed backlog entry with the intent as its `source:`. Triage never writes to `docs/backlog/` and never invokes a downstream skill. Triage renders no HTML artifact and stays in chat.
