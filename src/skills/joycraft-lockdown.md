...
> Lockdown mode is most useful when you already have tests in place -- it prevents the agent from modifying them while constraining behavior to writing code and running tests. Consider running `{{skill_prefix}}new-feature` first to set up a test-driven workflow, then come back to lock it down.
