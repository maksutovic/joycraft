...
Escalation gate first: a fact enforceable as architecture, a deny pattern, or a CI check goes to `{{skill_prefix}}harden`, not prose (advisory — continue if declined). Then, if `docs/context/` exists, quickly check whether this session revealed anything about:
...
Run the project's validation commands. Check {{boundary_file}} for project-specific commands. Common checks:
...
**Check {{boundary_file}} for "Git Autonomy" in the Behavioral Boundaries section.** If it says "STRICTLY ENFORCED" or the ALWAYS section includes "Push to feature branches immediately after every commit":
...
```
{{skill_prefix}}implement docs/features/2026-07-29-succinct-gates/specs/gate-contract-tests.md
