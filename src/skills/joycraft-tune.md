...
`docs/context/` through `{{skill_prefix}}add-fact` routing; the rest goes dormant,
...
Write to `docs{{skill_prefix}}assessment.md` AND display it: scores table, findings (evidence + gap + recommendation per dimension), upgrade plan (≤5 actions by impact).
