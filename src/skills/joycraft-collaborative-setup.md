...
> "No problem. The solo workflow needs no extra setup. Run `{{skill_prefix}}new-feature` when you want to start a feature."
...
If the user provided NEVER / ASK FIRST rules for the area, also write `docs/areas/<area-name>/boundaries.md` with the shared frontmatter and those rules. If they didn't, skip the boundaries file — the root {{boundary_file}} boundaries already cover the project-wide cases.
...
(Filled in during {{skill_prefix}}collaborative-setup — fill this in with your team's specific conventions.)
...
## Step 6: Trigger {{boundary_file}} Update

<!-- harness:claude -->
Now that `docs/areas/` exists, the next `npx joycraft upgrade` (or any future `npx joycraft init`) will pick it up and add the **Areas pointer** to {{boundary_file}} automatically — that pointer tells Claude "when working on the X area, read docs/areas/X/README.md first."
<!-- /harness -->
<!-- harness:codex|pi -->
Now that `docs/areas/` exists, the next `npx joycraft upgrade` (or any future `npx joycraft init`) will pick it up and add the **Areas pointer** to {{boundary_file}} automatically — that pointer tells the agent "when working on the X area, read docs/areas/X/README.md first."
<!-- /harness -->

Tell the user:

> "Run `npx joycraft upgrade` to refresh {{boundary_file}} with the Areas pointer (or `npx joycraft init` if you haven't initialized yet)."
