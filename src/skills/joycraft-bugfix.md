...
**Guard clause:** If this is clearly a new feature, redirect to `{{skill_prefix}}new-feature` and stop.
...
custom structure — `{{skill_prefix}}implement` executes against them. Treat the
...
**For large bugs that span multiple files/systems:** Consider whether this should be decomposed into multiple specs. If so, create a brief first using `{{skill_prefix}}new-feature`, then decompose. A bug fix spec should be implementable in a single session.
...
Next:
```bash
{{skill_prefix}}implement docs/bugfixes/<area>/bugfix-name.md
```
Run {{clear}} first.
