...
Before choosing a doc, apply the intervention-elimination hierarchy: **can this fact be enforced as architecture, a deny pattern, or a CI check?** A fact that is ALWAYS/NEVER-shaped, could cause real damage if violated, and applies across all work belongs in {{boundary_file}}'s behavioral boundaries with teeth — invoke `{{skill_prefix}}harden` to convert it (rule + deny pattern stamped together) and stop; prose is the residue after checks fail, not the default destination. Advisory, never blocking: if the human declines, or harden is not installed (note it), continue to Step 2. A check-eligible fact whose *why* deserves prose can still land in the decision log per Step 2's rubric. Purely informational facts, one-time decisions, and diagnostic tips are not escalation candidates — classify them below.
...
Escalated to {{skill_prefix}}harden:
