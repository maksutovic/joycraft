...
The user has a fact to capture. Your job is to classify it, route it to the correct context document, append it in the right format, and optionally add a {{boundary_file}} boundary rule.

## Step 1: Get the Fact

If the user already provided the fact (e.g., `{{skill_prefix}}add-fact the staging DB resets every Sunday`), use it directly.
...
- **Redundant with {{boundary_file}}** — the fact restates a rule {{boundary_file}} already carries. Example: "we never push to main."
...
Reject with a one-line reason and, where one exists, the fact's correct existing home ("already in {{boundary_file}} under NEVER"). Never silently drop a fact — the human can override and capture it anyway with the ban noted.
...
## Step 6: Evaluate {{boundary_file}} Boundary Rule

Decide whether the fact also warrants a rule in {{boundary_file}}'s behavioral boundaries:

**Add a {{boundary_file}} rule if the fact:**
...
**Do NOT add a {{boundary_file}} rule if the fact is:**
...
If a rule is warranted, read {{boundary_file}}, find the appropriate section (ALWAYS, ASK FIRST, or NEVER under Behavioral Boundaries), and append the rule. If no Behavioral Boundaries section exists, append one.
...
[If {{boundary_file}} was also updated:]
Added {{boundary_file}} rule:
