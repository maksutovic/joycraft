...
4. Group what you find:
   - **Drafts** (frontmatter `status: draft`) — likely from `{{skill_prefix}}interview`.
...
COMPLEX (5+ files, architectural decisions, unfamiliar area):
  → {{skill_prefix}}research — gather codebase facts before committing to a design
  → {{skill_prefix}}design — make architectural decisions explicit
  → Then execute specs

MEDIUM (clear scope but non-trivial):
  → {{skill_prefix}}design — make key decisions explicit before building
  → Then execute specs
...
1. Read the spec
2. Implement
3. Run {{skill_prefix}}session-end to capture discoveries
4. Commit and PR

Ready to start?
```

Before the Handoff block, run the deposition checkpoint: invoke `{{skill_prefix}}decide <brief path>`
...
You can also use `{{skill_prefix}}decompose` to re-decompose a brief if the breakdown needs adjustment, or run `{{skill_prefix}}interview` first for a lighter brainstorm before committing to the full workflow.
