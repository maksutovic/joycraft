...
// The canonical-pipeline transform (`{{var}}` substitution, `<!-- harness: -->`
// conditional blocks, `instructions:` frontmatter strip, and the unknown-variable
// build failure) is covered exhaustively as pure unit tests in
// tests/skill-template.test.ts. Those run with no I/O and no shared state.
//
// Earlier revisions of this file re-asserted the same behavior at the
// integration level by writing fixture skills into the real src/skills/ and
// per-harness dirs and shelling out to the generator. Because Vitest runs test
// FILES in parallel, that mutation raced sibling tests (codex/pi parity,
// bundled-files sync) that read those same dirs — an intermittent failure with
// zero added coverage. Removed in favor of the unit tests; do not reintroduce
// fixture-writing tests against the live source dirs here.
