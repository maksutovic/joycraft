...
Check for: AGENTS.md (with meaningful content), `docs/features/<slug>/` (briefs + specs), `docs/bugfixes/<area>/`, `docs/discoveries/`, `docs/context/*.md` fact-docs, `docs/context/reference/` long-form docs, `.omp/skills/`, and test configuration.
...
Read AGENTS.md and explore the project. Score each with specific evidence (1 = absent, 3 = partially there, 5 = comprehensive; credit substance over format):
...
| Skills & Hooks | `.omp/skills/` files, hooks config |
...
`docs/context/` through `/skill:joycraft-add-fact` routing; the rest goes dormant,
...
AGENTS.md carries a `<!-- joycraft:folder-map -->` block, diff its rows
...
Write to `docs/skill:joycraft-assessment.md` AND display it: scores table, findings (evidence + gap + recommendation per dimension), upgrade plan (≤5 actions by impact).
...
`docs/skill:joycraft-assessment.md` is written first and stays **canonical** —
...
Frame it with the levels: most projects should aim to run excellently at Levels 3-4 (spec-driven development with a well-maintained harness). Mention Level 5 (spec queue, autofix, holdout scenarios) once, as an experimental north star for teams with the budget and infrastructure to maintain it — not the expected next step. **Tip:** Run `/skill:joycraft-optimize` to audit your session's token overhead — plugins, MCP servers, and harness file sizes. **Tip:** If your memory file (AGENTS.md) predates the installed model profile (`docs/templates/reference/model-profile-claude-fable-5-1.md`), run `/skill:joycraft-optimize`. It flags legacy anti-formatting and hand-holding rules as `RETIRE` or `PROBATION` candidates and applies nothing.
