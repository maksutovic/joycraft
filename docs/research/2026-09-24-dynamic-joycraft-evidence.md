...
| Q2 customer | **Solo practitioners of mixed skill** (pending confirmation of order — see below) | "Me truly first as I am the super user of this product, but I now have about 15 colleagues all of use it daily driver, across different size teams, but if I can get it to work best for me, we can easily get it for others especially then I'll make content etc.. around into teach people, but the hope also being that as this system gets much easier house and more driven by the harness this becomes a non issue really" |
...
| Q8 output | **Intent + commit on a new feature branch** — amended (pending confirmation) | "no brief though, as you and I will have follow up discussion after I read the panel findings, so it really should be an in-depth html artifact with their findings, same for codex, you can make a markdown copy so easy to digest for you" |

**Pending confirmations (reversal rule):** Q2 — control says "practitioners of mixed skill", rationale says "me truly first"; the lead reads this as *maintainer first as the super-user, colleagues second, one path*. Q8 — control includes a draft brief, rationale says "no brief though"; the lead reads this as *intent + evidence pack committed, no brief, findings as an in-depth HTML artifact plus a Markdown copy*.
