...
export async function verifyConsumerInstallations({
  ...
  previousInitArgs = ['init'],
  // This temporary fixture contains only files generated by the prior package.
  // Review the legacy checker replacement explicitly; real projects keep their ownership guard.
  updateArgs = ['update', '--non-interactive', '--replace-customized', '.claude/hooks/joycraft-version-check.mjs'],
} = {}) {
  ...
  for (let attempt = 1; attempt <= Math.max(1, attempts); attempt += 1) {
    try {
      ...
      await adapter.exec({ spec: previousTarball, command: executable, args: previousInitArgs, cwd: updateRoot, cacheDir: warmCache, preferOnline: true });
