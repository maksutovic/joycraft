...
If AGENTS.md already has a `## Product Identity` section, skip this step entirely. Otherwise offer — never force — three short identity questions alongside the gap interview:
...
- **Zero-sum admission.** Every directional line added to AGENTS.md names the line it displaces — ideally an ALWAYS/NEVER prose rule converted to a deny pattern via /skill:joycraft-harden.
- **Behavioral check.** Before writing, pick 2–3 concrete behaviors the section should change. Ship small and dated, with a pre-committed review at the next /skill:joycraft-optimize run.
- **Rule-shaped answers aren't taste.** "Never use var" is a discipline rule — route it toward /skill:joycraft-harden per the zero-sum admission; keep only taste-shaped answers here.

Collected identity content joins the Step 4 batch as a dated `## Product Identity` section in AGENTS.md — Values / Glossary / Taste subsections, only the populated ones.
