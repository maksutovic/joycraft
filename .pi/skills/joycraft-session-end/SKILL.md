...
Escalation gate first: a fact enforceable as architecture, a deny pattern, or a CI check goes to `/skill:joycraft-harden`, not prose (advisory — continue if declined). Then, if `docs/context/` exists, quickly check whether this session revealed anything about:
