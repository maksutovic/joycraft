...
Every installed skill (`.github/skills/**/SKILL.md`) must declare `entry: human | agent | situational` in its frontmatter — this is presentation/discovery metadata only, not a flow change:
...
2. **Skill files** — glob `.github/skills/**/*.md`. Count lines per file. Threshold: ≤200 lines each.
