...
- **The dossier is display-only.** All capture happens in the native question
  UI (the AskUserQuestion tool) — never via interactive HTML, pick-strings, or
  paste-backs.
...
- **Path given** (`/joycraft-decide <brief-or-design path>`): use it. If it's
...
1. Read the brief's Open Questions and Resolved Decisions, plus the design
   doc if one exists.
2. Read the project boundaries: the ASK FIRST and NEVER lists in `CLAUDE.md`
   / `AGENTS.md`.
...
(mandatory boundary questions first). Mechanics that are load-bearing
(verified in `docs/discoveries/2026-07-20-question-capture.md`):

- **Every question has ≥2 real options.** A one-option question is invalid —
  the tool rejects it and the flow degrades to plain chat.
