...
<!-- PILOT: repo-local, not in src/ — see 2026-07-20-decision-dossier brief decision #7 -->
...
Read the project's boundary file (AGENTS.md, or CLAUDE.md if that's where boundaries live),
`.claude/settings.json` `permissions.deny`, and `.claude/hooks/joycraft/deny-patterns.txt`.
These are the **only two enforcement surfaces** you target. Both are generated by
`src/safeguard.ts` machinery — you append to the files it produces, you never edit
`src/safeguard.ts` itself, and you never add a new hook framework.
...
`verified` and just stamp provenance if the AGENTS.md line is missing it — don't
...
- Target only `permissions.deny` and `deny-patterns.txt` — never `src/safeguard.ts`,
  never `templates/`, never a new hook framework.
