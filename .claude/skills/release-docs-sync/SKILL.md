...
Joycraft auto-publishes on every merge to `main`, patch-bumping when the version wasn't set manually. Nothing in that pipeline touches prose. Docs therefore drift silently: by 0.7.4 the CHANGELOG still stopped at 0.7.1, and `CLAUDE.md`'s architecture map never learned that `src/copilot-skills/` existed.
...
Match each file's established voice. The CHANGELOG is narrative prose explaining *why* a change matters. `CLAUDE.md` is terse and directive. The README is instructional. Read the surrounding lines before adding to any of them.
...
CLAUDE.md     — added src/local-skills/ to architecture map
