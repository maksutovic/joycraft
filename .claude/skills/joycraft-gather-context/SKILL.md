...
If CLAUDE.md already has a `## Product Identity` section, skip this step entirely. Otherwise offer — never force — three short identity questions alongside the gap interview:
...
- **Zero-sum admission.** Every directional line added to CLAUDE.md names the line it displaces — ideally an ALWAYS/NEVER prose rule converted to a deny pattern via /joycraft-harden.
...
Collected identity content joins the Step 4 batch as a dated `## Product Identity` section in CLAUDE.md — Values / Glossary / Taste subsections, only the populated ones.
